...
/**
 * @brief  Clears the LCD display efficiently.
 * @param  lcd: Pointer to the LCD handle
 * @retval None
 */
...
/**
 * @brief  Clears a specific line on the LCD.
 * @param  lcd: Pointer to the LCD handle
 * @param  row: Row number to clear (0 or 1)
 * @retval None
 */
void lcd_clear_line(I2C_LCD_HandleTypeDef *lcd, int row)
{
    lcd_gotoxy(lcd, 0, row);
    for (int i = 0; i < 16; i++) {
        lcd_send_data(lcd, ' ');
    }
    lcd_gotoxy(lcd, 0, row);  // Return cursor to start of line
}
